...
from matrixcurator.config import get_logger
...
class MCPAwareLM(dspy.LM):
    ...
    """
    def forward(self, prompt: Optional[str] = None, messages: Optional[List[Dict[str, Any]]] = None, **kwargs):
        ...
        if session is not None:
            try:
                ...
                if loop.is_running():
                    _logger.warning("Cannot run sync MCP sampling inside an active event loop. Falling back to native dspy.LM.")
                ...
                _logger.warning(f"MCP sampling failed, falling back to native dspy.LM: {e}")
            except Exception as e:
                _logger.warning(f"Unexpected error during MCP sampling, falling back to native dspy.LM: {e}")
                
        # Fallback to native dspy.LM
        return super().forward(prompt=prompt, messages=messages, **kwargs)
        
    async def aforward(self, prompt: Optional[str] = None, messages: Optional[List[Dict[str, Any]]] = None, **kwargs):
        session = mcp_session_var.get()
        if session is not None:
            try:
                msgs = messages or [{"role": "user", "content": prompt}]
...
class CharacterExtraction(dspy.Signature):
    ...
    previous_errors: Optional[str] = dspy.InputField(desc="Errors from previous extraction attempts, if any")
    
    character_name: str = dspy.OutputField(desc="The name of the extracted character")
    states: List[CharacterState] = dspy.OutputField(desc="A list of states for the character")
...
class ExtractionEvaluation(dspy.Signature):
    ...
    document_text: str = dspy.InputField(desc="The parsed text of the document")
    extracted_data: Dict[str, Any] = dspy.InputField(desc="The extracted character data (name and states)")
    
    score: int = dspy.OutputField(desc="A score from 1 to 10 indicating the quality of the extraction (10 is perfect)")
    reasoning: str = dspy.OutputField(desc="Reasoning for the given score, highlighting any missing or incorrect information")
...
class ExtractionModule(dspy.Module):
    def __init__(self):
        ...
        weights_path = os.path.join(os.path.dirname(__file__), "..", "weights", "gemini-1.5-pro.json")
    ...
    def forward(self, document_text: str, character_index: int, previous_errors: Optional[str] = None):
        return self.extract(
