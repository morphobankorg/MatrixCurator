...
async def acompletion(*args, **kwargs) -> ModelResponse:
    ...
    if session is not None:
        try:
            ...
            logger.warning(f"MCP sampling failed, falling back to native litellm.acompletion: {e}")
        except Exception as e:
            logger.warning(f"Unexpected error during MCP sampling, falling back to native litellm.acompletion: {e}")
    ...
    if session is not None:
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("Cannot run sync MCP sampling inside an active event loop. Falling back to native litellm.completion.")
            ...
            logger.warning(f"MCP sampling failed, falling back to native litellm.completion: {e}")
        except Exception as e:
            logger.warning(f"Unexpected error during MCP sampling, falling back to native litellm.completion: {e}")
