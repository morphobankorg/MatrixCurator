...
async def agent_task(*, item: Any, df_docs: pd.DataFrame, **kwargs) -> str:
    ...
    final_state = await graph.ainvoke(
        initial_state,
        config={"configurable": {"thread_id": f"benchmark-{item.id}"}}
    )
...
def run_agents_benchmark():
    ...
    permutations = [
        ...
        (OrchestrationStrategy.STATIC_ROUTING, IntelligenceStrategy.PROGRAMMATIC_OPTIMIZATION),
        (OrchestrationStrategy.DYNAMIC_ROUTING, IntelligenceStrategy.PROMPT_ENGINEERING),
        (OrchestrationStrategy.DYNAMIC_ROUTING, IntelligenceStrategy.PROGRAMMATIC_OPTIMIZATION)
    ]
    
    def process_permutation(routing, intelligence):
        run_name = f"Agent-{routing.value}-{intelligence.value}"
        logger.info("Running Agent Benchmark", routing=routing.value, intelligence=intelligence.value)
