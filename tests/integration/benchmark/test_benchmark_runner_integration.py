...
from src.benchmark.core.runner import run_all
from src.benchmark.core.decorators import benchmark
import src.benchmark.core.registry
from src.benchmark.confbenchmark import benchmark_setup
...
async def test_parquet_clone_generation():
    ...
    with patch("src.benchmark.confbenchmark.os.path.exists", return_value=True):
        with patch("src.benchmark.confbenchmark.pd.read_parquet", return_value=mock_df):
            with patch("pandas.DataFrame.to_parquet") as mock_to_parquet:
                mock_tool = MagicMock()
                mock_tool.invoke.return_value = "Parsed TXT text"
                with patch("src.benchmark.confbenchmark.parse_with_txt", mock_tool):
                    # We are testing benchmark_setup(skip_sync=True)
                    await benchmark_setup(skip_sync=True, limit=1, lf_client=None)
                    
                    # Verify to_parquet is called once
                    mock_to_parquet.assert_called_once()
                    
                    # Verify the path ends with .cache/documents.parquet
                    called_path = mock_to_parquet.call_args[0][0]
                    assert ".cache" in called_path
                    assert "documents.parquet" in called_path
                    
                    # Verify that text column was populated with JSON array
                    saved_df = mock_to_parquet.call_args.args[0] if hasattr(mock_to_parquet.call_args, "args") else mock_df
                    # Actually mock_to_parquet is a method on DataFrame, so we can check mock_df directly since it's modified in place
                    
                    text_json = mock_df.iloc[0]["text"]
                    assert text_json is not None
                    
                    parsed_data = json.loads(text_json)
                    assert isinstance(parsed_data, list)
                    assert len(parsed_data) == 1
                    assert parsed_data[0]["parser"] == "txt"
                    assert parsed_data[0]["pages"][0]["content"] == "Parsed TXT text"
